...
class TestCliniqueManagement(unittest.TestCase):
    ...
    def test_insertrecord(self):
        ...
        for index in Patients:
            list = Patients[index]
            self.assertAlmostEqual(len(list) == 3)

    def test_insertrecord1(self):
        '''
        Description: Test the patient details that has been inserted to the file for negative test cases.
        Parameters: Patient details.
        Return: None.
        '''
        patientList = Patient()
        patientList.addPatient("s", "629", "null")
        Patients = patientList.getPatient()
        for index in Patients:
            list = Patients[index]
